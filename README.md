...
* optional themes, such as `github` (default), `github_high_contrast`, `dracula`, etc.
...
1. (Optional) Make other secrets for more configuration.
   ...
   - To optionally select from an existing theme, add the theme name,
     such as `catppuccin` for example, to a secret called `THEME`.
     Refer to  `theme.zig` file to view existing or add a new theme.
